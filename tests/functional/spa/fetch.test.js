...
testDriver.test(
  "capturing fetch in SPA interactions",
  supported,
  function (t, browser, router) {
    ...
    let loadPromise = browser.safeGet(
      router.assetURL("spa/fetch.html", { loader: "spa" })
    );

    rumPromise.then(({ query }) => {
      t.ok(
        query.af.split(",").indexOf("spa") !== -1,
        "should indicate that it supports spa"
      );
    });
    ...
    Promise.all([eventsPromise, rumPromise, loadPromise])
      ...
      .then(({ query, body }) => {
        ...
        t.equal(
          fetch.requestBodySize,
          3,
          "should have correct requestBodySize"
        );
        t.equal(
          fetch.responseBodySize,
          3,
        ...
        t.equal(
          fetch.requestedWith,
          "fetch",
          "should indicate it was requested with fetch"
        );

        let fixup = receiptTime - query.rst;
        let estimatedInteractionTimestamp = interactionTree.start + fixup;
        t.ok(
          estimatedInteractionTimestamp > testStartTime,
          "estimated ixn start after test start"
        );
        t.ok(
          estimatedInteractionTimestamp < receiptTime,
          "estimated ixn start before receipt time"
        );
      })
      .catch(fail);
...
testDriver.test("response size", supported, function (t, browser, router) {
  ...
  testCases.forEach(function (testCase) {
    t.test(testCase.name, function (t) {
      let loadPromise = browser.safeGet(
        router.assetURL(testCase.asset, { loader: "spa" })
      );
      let eventsPromise = router.expectEvents();
      let rumPromise = router.expectRum();
      ...
      Promise.all([rumPromise, loadPromise, eventsPromise])
        ...
        .then(({ query, body }) => {
          ...
          var fetchNode = interactionTree.children.find(
            (node) => node.type === "ajax"
          );
          t.equal(
            fetchNode.responseBodySize,
            testCase.responseBodySize,
            "should have correct responseBodySize"
          );
          t.end();
        })
