...
describe('Soft navigations', () => {
  ...
  it('multiple finished ixns retain the correct start/end timestamps in payload', async () => {
    await Promise.all([
      interactionsCapture.waitForResult({ totalCount: 1 }),
    ...
    let interactionHarvests = await interactionsCapture.waitForResult({ totalCount: 3 })

    console.log(interactionHarvests.map(x => x.request.body))
  })
