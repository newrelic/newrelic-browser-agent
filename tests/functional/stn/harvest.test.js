  ...
  function (t, browser, router) {
    ...
    Promise.all([resourcePromise, loadPromise, rumPromise])
      ...
      .then((result) => {
        ...
        t.ok(
          secondParsed.res.length > firstParsed.res.length,
          "second try has more nodes than first"
        );
        t.ok(
          containsAll(secondParsed, firstParsed),
          "all nodes have been resent"
        );
...
testDriver.test(
  "retried first harvest captures ptid",
  supported,
  function (t, browser, router) {
  ...
  function (t, browser, router) {
    ...
    Promise.all([resourcePromise, loadPromise, rumPromise])
      ...
      .then((result) => {
        ...
        t.ok(
          containsAll(thirdParsed, secondParsed),
          "all nodes have been resent"
        );
        ...
        t.ok(
          resentNodes.length === 0,
          "nodes from first successful harvest are not resent in second harvest"
        );

        resentNodes = intersectPayloads(thirdParsed, firstParsed);
        t.ok(
          resentNodes.length === 0,
          "nodes from first successful harvest are not resent in third harvest"
        );
