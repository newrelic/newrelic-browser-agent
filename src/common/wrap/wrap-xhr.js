...
export function wrapXhr (sharedEE) {
  ...
  const origSRH = OrigXHR.prototype.setRequestHeader
  ...
  XHR.prototype.setRequestHeader = function setRequestHeader (header, value) {
    if (header.toLowerCase() === NEW_RELIC_ENTITY_GUID_HEADER) {
      const context = ee.context(this)
      context.entityGuid = value
      return // do not allow the newrelic entity guid header to be assigned to the request, just use it for context later
    }
