...
export class Instrument extends InstrumentBase {
  ...
  constructor (agentRef, auto = true) {
    ...
    const { datasources, anyEnabled } = getEnabledDatasources(this.agentRef.init)

    if (isBrowserScope) {
      if (datasources.userActionsEnabled) {
      ...
      if (datasources.resourcesEnabled && globalScope.PerformanceObserver?.supportedEntryTypes.includes('resource')) {
    ...
    if (anyEnabled) this.importAggregator(agentRef)
