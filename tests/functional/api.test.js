...
testDriver.test(
  "customTransactionName 1 arg unload",
  withUnload,
  function (t, browser, router) {
    ...
    Promise.all([metricsPromise, rumPromise, loadPromise])
      .then(([{ body, query }]) => {
        const time = getTime(
          body ? JSON.parse(body)?.cm : JSON.parse(query.cm)
        );
        t.equal(
          query.ct,
          "http://custom.transaction/foo",
...
testDriver.test(
  "noticeError takes an error object",
  withUnload,
  function (t, browser, router) {
    t.plan(2);
...
testDriver.test(
  "finished records a PageAction when called before RUM message",
  function (t, browser, router) {
    let rumPromise = router.expectRum();
...
testDriver.test(
  "release api adds releases to jserrors",
  withUnload,
  function (t, browser, router) {
    let rumPromise = router.expectRum();
    let loadPromise = browser.get(
    ...
    Promise.all([rumPromise, loadPromise])
      ...
      .then(({ query, body }) => {
        t.equal(
          query.ri,
          '{"example":"123","other":"456"}',
          "should have expected value for ri query param"
        );
        t.end();
      })
      .catch(fail(t));
...
testDriver.test(
  "release api limits releases to jserrors",
  withUnload,
  function (t, browser, router) {
    let rumPromise = router.expectRum();
    let loadPromise = browser.get(
    ...
    Promise.all([rumPromise, loadPromise])
      ...
      .then(({ query, body }) => {
        ...
        t.deepEqual(
          queryRi,
          ri,
          `${JSON.stringify(ri)} is expected but got ${JSON.stringify(queryRi)}`
        );
        t.end();
      })
      .catch(fail(t));
...
testDriver.test(
  "release api limits release size to jserrors",
  withUnload,
  function (t, browser, router) {
    let rumPromise = router.expectRum();
    let loadPromise = browser.get(
...
testDriver.test(
  "no query param when release is not set",
  withUnload,
  function (t, browser, router) {
    let rumPromise = router.expectRum();
    let loadPromise = browser.get(
...
testDriver.test(
  "api is available when sessionStorage is not",
  function (t, browser, router) {
    let rumPromise = router.expectRum();
    let loadPromise = browser.get(
      router.assetURL("api/session-storage-disallowed.html")
    );
      ...
      .then(() =>
        browser.waitFor(
          asserters.jsCondition(
            "typeof window.newrelic.addToTrace === 'function'"
          )
        )
      )
      .then((result) => {
