...
beforeEach(async () => {
  ...
  softNavAggregate.events = new EventBuffer()
...
test('.save forcibly harvest any would-be cancelled ixns', async () => {
  ...
  await new Promise(process.nextTick)
  expect(softNavAggregate.events.buffer.length + softNavAggregate.events.held.buffer.length).toEqual(3)
})
...
test('.onEnd queues callbacks for right before ixn is done', async () => {
  ...
  expect(hasRan).toEqual(true)
  expect(softNavAggregate.events.buffer.length).toEqual(1) // ixn was discarded
})
...
test('multiple finished ixns retain the correct start/end timestamps in payload', () => {
  ...
  expect(softNavAggregate.events.buffer.length).toEqual(3)
