...
testDriver.test(
  "referrer attribute is sent in the query string",
  notSafariWithSeleniumBug,
...
testDriver.test(
  "referrer sent in query does not include query parameters",
  notSafariWithSeleniumBug,
  function (t, browser, router) {
    t.plan(1);
    let loadPromise = browser.safeGet(router.assetURL("instrumented.html"));
...
testDriver.test(
  "referrer sent in referer header includes path",
  originOnlyReferer.inverse().and(notSafariWithSeleniumBug),
  function (t, browser, router) {
    t.plan(1);
    let loadPromise = browser.safeGet(router.assetURL("instrumented.html"));
    let rumPromise = router.expectRum();
    ...
    Promise.all([rumPromise, loadPromise])
      .then(([{ query, headers }]) => {
        ...
        t.ok(
          headerUrl.query != null,
          "url in referer header contains query parameters"
        );
      })
      .catch(fail(t, FAIL_MSG));
  }
      ...
      Promise.all([rumPromise, loadPromise])
        .then(([{ query, headers }]) => {
          ...
          t.equal(
            headerUrl.pathname,
            redirectedPath,
            "referer header contains the redirected URL"
          );
        })
        .catch(fail(t, FAIL_MSG));
    });
    ...
    t.test("query param", function (t) {
      ...
      Promise.all([rumPromise, loadPromise])
        .then(([{ query, headers }]) => {
          var queryRefUrl = url.parse(query.ref);
          t.equal(
            queryRefUrl.pathname,
            redirectedPath,
            "ref param contains the redirected URL"
          );
        })
        .catch(fail(t, FAIL_MSG));
    });
...
testDriver.test(
  "when url is changed using replaceState during load",
  notSafariWithSeleniumBug,
  function (t, browser, router) {
...
testDriver.test(
  "browsers that do not decode the url when accessing window.location encode special characters in the referrer attribute",
  notSafariWithSeleniumBug,
  function (t, browser, router) {
    t.plan(2);
    ...
    Promise.all([rumPromise, loadPromise])
      .then(([{ query }]) => {
        ...
        t.equal(
          query.ref,
          cleanAssetURL,
          "The ref attribute should be the same as the assetURL"
        );
      })
      .catch(fail(t, FAIL_MSG));
  }
...
testDriver.test(
  "cookie disabled: query string attributes",
  notSafariWithSeleniumBug,
  function (t, browser, router) {
    t.plan(2);
...
testDriver.test(
  "cookie enabled by default: query string attributes",
  notSafariWithSeleniumBug,
  function (t, browser, router) {
    t.plan(2);
    let loadPromise = browser.safeGet(router.assetURL("instrumented.html"));
