...
testDriver.test(
  "onreadystatechange only called once with zone.js",
  supported,
  function (t, browser, router) {
    ...
    Promise.all([eventsPromise, rumPromise, loadPromise])
      .then(([eventsResult]) => {
        ...
        const interactionAttr = interactionTree.children.find(
          (x) => x.key === "counts"
        );
        ...
        counts.forEach((c, i) => {
          if (i && i % 2 === 0)
            t.ok(c === 1, "state 2 and 4 should be called exactly once");
