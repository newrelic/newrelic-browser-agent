...
testDriver.test(
  "DT payload is NOT added when the feature is not enabled (default)",
  supported,
  function (t, browser, router) {
    ...
    let loadPromise = browser.safeGet(
      router.assetURL("spa/dt/fetch-dt-harvest-disabled.html", {
        loader: "spa",
...
testDriver.test(
  "fetch request using string URL with two parameters on same origin has AJAX request with DT payload",
  supported,
  function (t, browser, router) {
    t.plan(4);
    ...
    let loadPromise = browser.safeGet(
      router.assetURL(
        "spa/dt/fetch-dt-harvest-enabled-stringurl-two-params.html",
        { loader: "spa", injectUpdatedLoaderConfig: true }
      )
    );
...
testDriver.test(
  "fetch request using string URL with one parameter on same origin has AJAX request with DT payload",
  supported,
  function (t, browser, router) {
    t.plan(4);
    ...
    let loadPromise = browser.safeGet(
      router.assetURL(
        "spa/dt/fetch-dt-harvest-enabled-stringurl-one-param.html",
        { loader: "spa", injectUpdatedLoaderConfig: true }
      )
    );
...
testDriver.test(
  "fetch request using object URL on same origin has AJAX request with DT payload",
  supported,
  function (t, browser, router) {
    t.plan(4);
    ...
    let eventsPromise = router.expectEvents();
    let loadPromise = browser.safeGet(
      router.assetURL("spa/dt/fetch-dt-harvest-enabled-objecturl.html", {
        loader: "spa",
        injectUpdatedLoaderConfig: true,
      })
    );
...
testDriver.test(
  "fetch request on different origin has no AJAX request with DT payload",
  supported,
  function (t, browser, router) {
    t.plan(4);
    ...
    let eventsPromise = router.expectEvents();
    let loadPromise = browser.safeGet(
      router.assetURL("spa/dt/fetch-dt-harvest-enabled-different-origin.html", {
        loader: "spa",
        injectUpdatedLoaderConfig: true,
      })
    );
