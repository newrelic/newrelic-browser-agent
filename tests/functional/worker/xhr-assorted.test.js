...
function addEventListenerPatched(type, browserVersionMatcher) {
  testDriver.test(
    `${type} - xhr instrumentation works with EventTarget.prototype.addEventListener patched`,
...
function constructorMonkeyPatched(type, browserVersionMatcher) {
  testDriver.test(
    `${type} - xhr instrumentation works with bad XHR constructor monkey-patch`,
    browserVersionMatcher,
    function (t, browser, router) {
      let assetURL = router.assetURL(`worker/${type}-worker.html`, {
        init: {
...
function catCors(type, browserVersionMatcher) {
  testDriver.test(
    `${type} - does not set CAT headers on outbound XHRs to different origin`,
    browserVersionMatcher,
    function (t, browser, router) {
      let assetURL = router.assetURL(`worker/${type}-worker.html`, {
        workerCommands: [
...
function harvestRetried(type, browserVersionMatcher) {
  testDriver.test(
    `${type} - ajax events harvests are retried when collector returns 429`,
    browserVersionMatcher,
    function (t, browser, router) {
      let assetURL = router.assetURL(`worker/${type}-worker.html`, {
        init: {
...
function abortCalled(type, browserVersionMatcher) {
  testDriver.test(
    `${type} - xhr.abort() called in load callback`,
    browserVersionMatcher,
    function (t, browser, router) {
      let assetURL = router.assetURL(`worker/${type}-worker.html`, {
        init: {
