...
describe('soft navigations', () => {
  ...
  describe('aggregate', () => {
    ...
    test('getInteractionFor grabs the right active interaction for a timestamp', () => {
      ...
      softNavAggregate.interactionsToHarvest.hold()
      ...
      softNavAggregate.interactionsToHarvest.held.buffer[0].status = 'finished'
      softNavAggregate.interactionsToHarvest.unhold()
    ...
    test('interactions are backed up when pre harvesting', () => {
      ...
      expect(softNavAggregate.interactionsToHarvest.held.hasData).toEqual(true)
      expect(softNavAggregate.interactionsToHarvest.hasData).toEqual(false)
    })
    ...
    describe('back up buffer is cleared when', () => { // prevent mem leak
      ...
      test('harvest was blocked', () => {
        ...
        expect(softNavAggregate.interactionsToHarvest.held.hasData).toEqual(false)
      })
      test('harvest is sent but got a retry response', () => {
        softNavAggregate.onHarvestFinished({ sent: true, retry: true })
        expect(softNavAggregate.interactionsToHarvest.held.hasData).toEqual(false)
        expect(softNavAggregate.interactionsToHarvest.hasData).toEqual(true)
      })
