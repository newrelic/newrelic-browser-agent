...
testDriver.test(
  "final harvest doesnt append pageHide if already previously recorded",
  workingSendBeacon,
  function (t, browser, router) {
...
testDriver.test(
  ...
  reliableResourcesHarvest.and(stnSupported),
  function (t, browser, router) {
    let url = router.assetURL("final-harvest.html");
    let loadPromise = browser.safeGet(url).catch(fail);
    let rumPromise = router.expectRum();
...
testDriver.test(
  "final harvest sends ajax events",
  workingSendBeacon.and(doNotSupportWaitForConditionInBrowser),
  function (t, browser, router) {
    let url = router.assetURL("final-harvest-ajax.html", {
      loader: "spa",
      init: { ajax: { enabled: true } },
    });
    let loadPromise = browser.safeGet(url).catch(fail);
    let rumPromise = router.expectRum();
