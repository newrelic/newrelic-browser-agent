...
beforeEach(async () => {
  runtimeModule = await import('../constants/runtime')
  wrapFunctionModule = await import('./wrap-function')
...
test('should not wrap when getPrototypeOf is not supported', async () => {
  ...
  ;(await import('./wrap-events')).wrapEvents()
...
test('should send event when adding event listener', async () => {
  ...
  const eventsEmitter = (await import('./wrap-events')).wrapEvents()
