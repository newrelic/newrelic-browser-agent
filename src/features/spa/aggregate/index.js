...
export class Aggregate extends AggregateBase {
  ...
  constructor(agentIdentifier, aggregator) {
    ...
    this.state = {
      ...
      depth: 0,
      harvestTimeSeconds:
        getConfigurationValue(agentIdentifier, "spa.harvestTimeSeconds") || 10,
    ...
    register(
      ...
      function saveId(args, obj, timerId) {
        if (!state.currentNode || state.timerBudget - this.timerDuration < 0)
          return;
    ...
    register(
      ...
      function () {
        ...
        if (node) {
          ...
          if (!!this.currentNode && !!this.currentNode.interaction)
            this.currentNode.interaction.checkFinish();
        }
      },
      this.featureName,
      baseEE
    ...
    register(
      ...
      function (url) {
        if (state.currentNode) {
          var node = (this[JSONP_NODE] = state.currentNode.child(
            "ajax",
            this[FETCH_START]
          ));
    ...
    register(
      FETCH_START,
      function (fetchArguments, dtPayload) {
    ...
    register(
      ...
      function (t) {
        var interaction;
        if (state?.currentNode?.[INTERACTION])
          interaction = this.ixn = state.currentNode[INTERACTION];
        else if (
          state?.prevNode?.end === null &&
          state?.prevNode?.[INTERACTION]?.root?.[INTERACTION]?.eventName !=
            "initialPageLoad"
        )
    ...
    register(
      ...
      function (t, currentRouteName) {
        ...
        if (state.currentNode)
          state.currentNode[INTERACTION].setNewRoute(currentRouteName);
      },
      this.featureName,
      baseEE
    );
    ...
    function activeNodeFor(interaction) {
      return state.currentNode && state.currentNode[INTERACTION] === interaction
        ? state.currentNode
        : interaction.root;
    }
    ...
    function setCurrentNode(newNode) {
      if (!state.pageLoaded && !newNode && state.initialPageLoad)
        newNode = state.initialPageLoad.root;
      if (state.currentNode) {
      ...
      state.currentNode =
        newNode && !newNode[INTERACTION].root.end ? newNode : null;
    }
    ...
    function onHarvestStarted(options) {
      ...
      var payload = serializer.serializeMultiple(
    ...
    function saveInteraction(interaction) {
      if (
        interaction.ignored ||
        (!interaction.save && !interaction.routeChange)
      ) {
      ...
      if (interaction.root.attrs.trigger === "initialPageLoad") {
        ...
        interaction.root.attrs.firstContentfulPaint =
          paintMetrics["first-contentful-paint"];
      }
