...
const {
  validatePageActionData,
  fail,
} = require("../ins/ins-internal-help.cjs"); // shared helpers
...
function paSubmission(type, supportRegOrESMWorker) {
  testDriver.test(
    `${type} - addPageAction sends PA event`,
    supportRegOrESMWorker,
...
function paRetry(type, supportRegOrESMWorker) {
  testDriver.test(
    `${type} - PageActions are retried when collector returns 429`,
    supportRegOrESMWorker,
    function (t, browser, router) {
      let assetURL = router.assetURL(`worker/${type}-worker.html`, {
        init: {
        ...
        workerCommands: [
          `newrelic.addPageAction("exampleEvent", {param: "value"})`,
        ],
      });
...
function paPrecedence(type, supportRegOrESMWorker) {
  testDriver.test(
    `${type} - addPageAction has precedence over custom attributes`,
    supportRegOrESMWorker,
    function (t, browser, router) {
      let assetURL = router.assetURL(`worker/${type}-worker.html`, {
        init: {
      ...
      function precValidatePageActionData(pageActionData) {
        ...
        t.equal(
          event.eventType,
          "PageAction",
          "defaults has correct precedence"
        );
        t.equal(event.browserHeight, 777, "att has correct precedence");
        t.equal(
          event.referrerUrl,
          "http://test.com",
          "attributes has correct precedence"
        );
        t.equal(
          event.foo,
          '{"bar":"baz"}',
          "custom member of attributes passed through"
        );
      }
    }
  );
