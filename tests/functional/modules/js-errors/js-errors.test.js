...
testDriver.test(
  "Error objects are sent via noticeError from core module",
  supported,
  function (t, browser, router) {
    t.plan(2);

    let loadPromise = browser.safeGet(
      router.assetURL("modular/js-errors/error-obj.html", opts)
    );
      ...
      .then(([loadResult, errorsResult]) => {
        ...
        if (params) {
          ...
          t.equal(
            "Error",
            exceptionClass,
            "The agent passes an error class instead of a string."
          );
...
testDriver.test(
  ...
  function (t, browser, router) {
    ...
    let loadPromise = browser.safeGet(
      router.assetURL("modular/js-errors/string.html", opts)
    );
    let errorsPromise = router.expectErrors();
...
testDriver.test(
  ...
  function (t, browser, router) {
    ...
    let loadPromise = browser.safeGet(
      router.assetURL("modular/js-errors/custom-attributes.html", opts)
    );
    let errorsPromise = router.expectErrors();
...
testDriver.test(
  ...
  function (t, browser, router) {
    ...
    let loadPromise = browser.safeGet(
      router.assetURL("modular/js-errors/multiple-instances.html", opts)
    );
      ...
      .then(([loadResult, ...errorsResult]) => {
        errorsResult.forEach((errorResult) => {
          ...
          if (params) {
            ...
            t.equal(
              "agent" + errorResult.query.a,
              message,
              "Params contain the right error message."
            );
          } else {
            fail("No error data was received.");
          }
...
testDriver.test(
  ...
  function (t, browser, router) {
    ...
    let loadPromise = browser.safeGet(
      router.assetURL("modular/js-errors/auto.html", opts)
    );
    let errorsPromise = router.expectErrors();
...
testDriver.test(
  "encoding error where message contains a circular reference",
  supported,
  function (t, browser, router) {
    t.plan(2);

    let loadPromise = browser.get(
      router.assetURL("modular/js-errors/circular.html", opts)
    );
    let errorsPromise = router.expectErrors();
    ...
    Promise.all([loadPromise, errorsPromise])
      .then(([loadResult, errorsResult]) => {
        ...
        t.equal(
          ...
          expectedErrorForBrowser(browser),
    ...
    function expectedErrorForBrowser(browser) {
      if (browser.match("ie@<11, edge")) {
        ...
        browser.match(
          "chrome, firefox@>=35, ie@11, android@>=4.4, safari@>=10, ios@>12"
        )
      ) {
...
testDriver.test(
  "reporting errors from event listener callbacks",
  supported,
  function (t, browser, router) {
    let assetURL = router.assetURL(
      "modular/js-errors/event-listener.html",
      opts
    );
    ...
    Promise.all([errorsPromise, loadPromise])
      .then(([errorsResponse]) => {
        ...
        let eventListenersURL = router
          .assetURL("js/event-listener-error.js")
          .split("?")[0];
...
testDriver.test(
  "reporting uncaught errors from external scripts",
  supported,
  function (t, browser, router) {
    let errorsPromise = router.expectErrors();
    let loadPromise = browser.get(
      router.assetURL("modular/js-errors/external-uncaught-error.html", opts)
    );
...
testDriver.test(
  "reporting uncaught errors from inline scripts",
  supported,
  function (t, browser, router) {
    let errorsPromise = router.expectErrors();
    let loadPromise = browser.get(
      router.assetURL("modular/js-errors/inline-uncaught-error.html", opts)
    );
...
testDriver.test(
  "reporting errors from setInterval callbacks",
  supported,
  function (t, browser, router) {
    let assetURL = router.assetURL(
      "modular/js-errors/set-interval-error.html",
      opts
    );
...
testDriver.test(
  "reporting errors from setTimeout callbacks",
  supported,
  function (t, browser, router) {
    let assetURL = router.assetURL(
      "modular/js-errors/set-timeout-error.html",
      opts
    );
    ...
    Promise.all([errorsPromise, loadPromise])
      .then(([response]) => {
        ...
        t.ok(
          browser.safeEval("window.setTimeoutFired", "window.setTimeoutFired")
        );
        let expectedErrors = [
          {
...
testDriver.test(
  "reporting errors from XHR callbacks",
  supported,
  function (t, browser, router) {
...
testDriver.test(
  "Errors are not sent if agent is not initialized",
  supported,
  function (t, browser, router) {
    ...
    let loadPromise = browser
      ...
      .safeGet(
        router.assetURL("modular/js-errors/invalid-not-initialized.html", opts)
      );
    let errorsPromise = router.expectErrors();
    ...
    function fail() {
      t.ok(
        true,
        "Errors Promise did not execute because agent was not initialized"
      );
      t.end();
    }
...
testDriver.test(
  "Errors are not sent if feature is disabled",
  supported,
  function (t, browser, router) {
    setRouterTimeout(10000);
    t.plan(1);
    ...
    function fail() {
      t.ok(
        true,
        "Errors Promise did not execute because errors feature was disabled"
      );
      t.end();
    }
