...
;[browserAgent, microAgent].forEach((agent) => {
  ...
  expectType<(name: string, options?: {start?: number | PerformanceMark, end?: number | PerformanceMark, customAttributes?: object}) => ({start: number, end: number, duration: number, customAttributes: object})>(agent.measure)
