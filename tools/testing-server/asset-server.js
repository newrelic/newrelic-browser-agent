...
class AssetTransform {
  ...
  execute() {
    throw new Error(
      "execute method of transforms must be implemented in subclasses"
    );
  }
}
...
class AgentInjectorTransform extends AssetTransform {
  constructor(buildDir, assetServer, router) {
    ...
    this.polyfills = runnerArgs.polyfills
      ? fs.readFileSync(`${this.buildDir}/nr-polyfills.min.js`)
      : "";
  }
  ...
  generateConfig(loaderName, params, ssl, injectUpdatedLoaderConfig) {
    let loaderSpec = [
      ...loaders,
      ...[{ name: "mfe" }, { name: "prebuilt" }],
    ].find((spec) => spec.name === loaderName);
  ...
  generateConfigString(loaderName, params, ssl, injectUpdatedLoaderConfig) {
    ...
    let loaderConfigAssignment = injectUpdatedLoaderConfig
  ...
  getLoaderContent(loader, dir, callback) {
    let loaderFilename = `nr-loader-${loader}${
      runnerArgs.polyfills ? "-polyfills" : ""
    }.min.js`;
  ...
  execute(params, assetPath, ssl, callback) {
    fs.readFile(assetPath, "utf-8", async (err, rawContent) => {
      ...
      let injectUpdatedLoaderConfig =
        params.injectUpdatedLoaderConfig === "true";

      const htmlPackageTags = [
        ...rawContent.matchAll(/{tests\/assets\/js\/internal\/.*}/g),
      ].map((x) => x[0]);
      ...
      this.getLoaderContent(loaderName, this.buildDir, (err, loaderContent) => {
        ...
        let configContent = "";
        try {
          configContent = this.generateConfigString(
            loaderName,
            params,
            ssl,
            injectUpdatedLoaderConfig
        ...
        if (params.scriptString) {
          try {
            scriptStringContent = Buffer.from(
              params.scriptString,
              "base64"
            ).toString();
          } catch (e) {
        ...
        let disableSsl =
          "window.NREUM||(NREUM={});NREUM.init||(NREUM.init={});NREUM.init.ssl=false;";
        ...
        let rspData = rawContent
          ...
          .replace(
            "{init}",
            tagify(disableSsl + initContent + this.getAjaxDenyListString())
          )
          .replace("{worker-commands}", tagify(disableSsl + wcContent))
          .replace(
            "{script}",
            `<script type="text/javascript" src="${params.script}" charset="utf-8"></script>`
          )
          .replace(
            "{script-injection}",
            `<script type="text/javascript">${scriptStringContent}</script>`
          );

        if (runnerArgs.polyfills) {
          rspData = rspData.replace(
            "{polyfills}",
            `<script type="text/javascript">${this.polyfills}</script>`
          );
        }
        ...
        if (!!htmlPackageTags.length && !!packageFiles.length) {
          packageFiles.forEach((pkg) => {
            ...
            rspData = rspData.replace(
              tag,
              tagify(disableSsl + UglifyJS.minify(pkg.data).code)
            );
          });
        }
...
const testRoutes = [
  ...
  new Route("GET", "/web-worker-agent", (req, res) => {
    fs.readFile(
      path.resolve(__dirname, "../../build/nr-loader-worker.min.js"),
      "utf-8",
      (err, data) => {
...
class AssetServer extends BaseServer {
  constructor(
    ...
    this.agentTransform = new AgentInjectorTransform(
      this.buildDir,
      this,
      this.router
    );
      ...
      transform.execute(params, assetPath, ssl, (err, transformed) => {
        if (err)
          return this.writeError(
            rsp,
            `Error while transforming asset ${err}: ${err.stack}`
          );
        rsp.writeHead(200, {
