...
var supported = testDriver.Matcher.withFeature("cors").exclude("ie@<=9"); // IE 9 and below do not support btoa()

testDriver.test(
  "DT headers are NOT added when the feature is not enabled (default)",
  supported,
  function (t, browser, router) {
    ...
    let loadPromise = browser.get(
      router.assetURL(
        "spa/dt/xhr-dt-sameorigin-load.html",
        { testId: router.testID, injectUpdatedLoaderConfig: true, config },
        true
      )
    );
    let meowPromise = router.expectCustomGet("/dt/{key}", (req, res) => {
      res.end("ok");
    });
    ...
    Promise.all([meowPromise, loadPromise])
      .then(([{ headers }]) => {
        ...
        t.notOk(
          headers["traceparent"],
          "traceparent header should not be present"
        );
        t.notOk(
          headers["tracestate"],
          "tracestate header should not be present"
        );
      })
      .catch(fail);
...
testDriver.test(
  "XHR request on same origin has DT headers",
  supported,
  function (t, browser, router) {
    let config = {
      accountID: "1234",
    ...
    Promise.all([meowPromise, loadPromise])
      .then(([{ headers }]) => {
        ...
        t.equal(
          parts[0],
          "00",
          "first part should be format version set to 00"
        );
        ...
        t.equal(
          key,
          config.trustKey + "@nr",
          "key should be in the right format"
        );
        ...
        t.equal(
          parts[5],
          "",
          "fifth part is empty - no transaction in Browser"
        );
        t.equal(
          parts[6],
          "",
...
testDriver.test(
  "XHR request on same origin has no newrelic header when disabled in configuration",
  supported,
  function (t, browser, router) {
    t.plan(3);
    let config = {
...
testDriver.test(
  "XHR request on different origin has no DT headers",
  supported,
  function (t, browser, router) {
    t.plan(3);
    let config = {
    ...
    let loadPromise = browser.get(
      router.assetURL("spa/dt/xhr-dt-crossorigin-load.html", {
        testId: router.testID,
        injectUpdatedLoaderConfig: true,
        config,
        init,
...
testDriver.test(
  "default headers on XHR request to allowed cross-origin call",
  supported,
  function (t, browser, router) {
    t.plan(3);
    let config = {
    ...
    let init = {
      distributed_tracing: {
        enabled: true,
        allowed_origins: [router.beaconURL()],
      },
      metrics: {
        enabled: false,
      },
...
testDriver.test(
  "headers configuration for cross-origin calls is respected",
  supported,
  function (t, browser, router) {
    t.plan(3);
    let config = {
    ...
    Promise.all([meowPromise, loadPromise])
      .then(([{ headers }]) => {
        t.ok(
          headers["newrelic"] == null,
          "newrelic header should not be present"
        );
        t.ok(
          headers["traceparent"] != null,
        ...
        t.ok(
          headers["tracestate"] != null,
          "tracestate header should be present"
        );
      })
      .catch(fail);
