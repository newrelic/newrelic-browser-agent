 ...
 */
 let denyList: Array<{ hostname: string; pathname: string }> = []
...
export function shouldCollectEvent(params: XhrParams): boolean {
  ...
  */
 export function setDenyList(denyListConfig: string[]) {
   denyList = []
  ...
  */
function domainMatchesPattern(pattern: string, domain: string): boolean {
  ...
  */
 function comparePath(pattern: string, path: string): boolean {
