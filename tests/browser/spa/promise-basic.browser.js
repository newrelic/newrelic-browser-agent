...
jil.browserTest("stringifying promises still works", function (t) {
  require("./helpers");
  t.ok(
    String(window.Promise).match(
      /function Promise\s*\(\)\s*{\s*\[native code\]\s*}/
    )
  );
  t.ok(
    window.Promise.toString().match(
      /function Promise\s*\(\)\s*{\s*\[native code\]\s*}/
    )
  );
  t.end();
  ...
  function onInteractionStart(cb) {
    new Promise(function (resolve, reject) {
      setTimeout(
        newrelic
          .interaction()
          .createTracer("timer-in-first-promise", function () {
            resolve(1);
          }),
        1
      );
      ...
      .then(() => {
        return new Promise(function (resolve, reject) {
          setTimeout(
            newrelic
              .interaction()
              .createTracer("timer-in-second-promise", function () {
                reject(2);
              }),
            2
          );
...
jil.browserTest("instanceof", function diferTest(t) {
  ...
  t.ok(
    promise instanceof unwrappedPromise,
    "instanceof should work on unwrapped Promise"
  );
  t.ok(
    unwrapped instanceof Promise,
    "instanceof should work on wrapped Promise"
  );
  t.ok(
    unwrapped instanceof unwrappedPromise,
    "instanceof should work on unwrapped Promise"
  );
  t.end();
});
      ...
      setTimeout(
        newrelic.interaction().createTracer("timer", function () {
          t.equal(
            val,
            thrownError,
            "promise should yield the error that was thrown in executor"
          );
