    ...
    Promise.all([resourcePromise, loadPromise, rumPromise])
      ...
      .then(([result]) => {
        ...
        const loadNodes = harvestBody.filter(function (node) {
          return (
            (node.t === "event" && node.n === "load") ||
            node.n === "readystatechange"
          );
        });
        t.notOk(
          loadNodes.length > 0,
          "XMLHttpRequest nodes not captured when ajax instrumentation is disabled"
        );
  ...
  function (t, browser, router) {
    ...
    Promise.all([resourcePromise, ajaxPromise, loadPromise, rumPromise])
      ...
      .then(([result]) => {
        ...
        t.ok(
          loadNodes.length > 0,
          "XMLHttpRequest nodes captured even with ajax deny list"
        );
