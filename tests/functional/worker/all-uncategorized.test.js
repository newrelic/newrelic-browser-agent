...
function apiAddReleaseTooMany(type, browserVersionMatcher) {
  testDriver.test(
    `${type} - release api adds tags to jserrors (with tags limit)`,
...
function apiAddReleaseTooLong(type, browserVersionMatcher) {
  testDriver.test(
    `${type} - release api limits the length of tags`,
    browserVersionMatcher,
    function (t, browser, router) {
      let assetURL = router.assetURL(`worker/${type}-worker.html`, {
        init: {
...
function apiAddReleaseNotUsed(type, browserVersionMatcher) {
  testDriver.test(
    `${type} - no query param when release is not set`,
    browserVersionMatcher,
    function (t, browser, router) {
      let assetURL = router.assetURL(`worker/${type}-worker.html`, {
        init: {
...
function harvestReferrerSent(type, browserVersionMatcher) {
  testDriver.test(
    `${type} - referrer attr is sent in the query string & does not include query params`,
    browserVersionMatcher,
    function (t, browser, router) {
      let assetURL = router.assetURL(`worker/${type}-worker.html`, {
        init: {
...
function harvestSessionIsNullWhenEnabled(type, browserVersionMatcher) {
  testDriver.test(
    `${type} - session tracking (enabled by default) is in query string attributes and is 0`,
    browserVersionMatcher,
    function (t, browser, router) {
      let assetURL = router.assetURL(`worker/${type}-worker.html`, {
        init: {
