...
export function initializeFeatures(agentIdentifier: string, api: Api, sharedAggregator: Aggregator, features: Features) {
    ...
    return Promise.all(features.getEnabledFeatures().map(async feature => {
        if (feature.auto) {
            const { Instrument }: { Instrument: any } = await import(`@newrelic/browser-agent-core/src/features/${feature.name}/instrument`) // dont lazy load this -- put it in main bundle
            const featureInstrumentation = new Instrument(agentIdentifier)
        }
        const { Aggregate }: { Aggregate: any } = await import(`@newrelic/browser-agent-core/src/features/${feature.name}/aggregate`)
