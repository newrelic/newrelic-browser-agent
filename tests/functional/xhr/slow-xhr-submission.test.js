...
var supported = testDriver.Matcher.withFeature("reliableUnloadEvent").exclude(
  "ie@<9"
); // need addEventListener too

testDriver.test(
  "slow XHR submission should not delay next page load",
  supported,
    ...
    Promise.all([rumPromise, loadPromise])
      .then(() => {
        ...
        if (browser.match("firefox@<31")) {
          ...
          return Promise.all([navigatePromise, router.expectXHRMetrics()]).then(
            ([feat, err]) => err
          );
        }
      })
      .then(() => {
        t.notok(
          routerResponded,
          "next page should have loaded before router responded"
        );
      })
      .catch(fail);
