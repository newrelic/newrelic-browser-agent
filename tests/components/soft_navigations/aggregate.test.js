...
test('processes regular interactions', () => {
  ...
  expect(softNavAggregate.events.buffer.length).toEqual(1)
  ...
  expect(softNavAggregate.events.buffer.length).toEqual(2)
  expect(softNavAggregate.events.buffer[1].end).toEqual(348.5) // check end time for the ixn is as expected
})
...
test('regular interactions have applicable timeouts', async () => {
  ...
  expect(softNavAggregate.interactionInProgress).toBeNull()
  expect(softNavAggregate.events.buffer.length).toEqual(0) // since initialPageLoad ixn hasn't closed, and we expect that UI ixn to have been cancelled
...
test('getInteractionFor grabs the right active interaction for a timestamp', () => {
  ...
  expect(softNavAggregate.getInteractionFor(currentTime)).toBe(softNavAggregate.events.buffer[0]) // queued+completed UI interaction is STILL chosen over initialPageLoad

  softNavAggregate.events.buffer[0].status = 'cancelled'
  ...
  const holdIxn = softNavAggregate.events.buffer[softNavAggregate.events.buffer.length - 1]
  ...
  expect(softNavAggregate.getInteractionFor(currentTime)).toBe(softNavAggregate.events.buffer[0])
})
...
test('interactions are backed up when pre harvesting', () => {
  ...
  expect(softNavAggregate.events.buffer.length).toEqual(0)
  expect(softNavAggregate.events.held.buffer.length).toEqual(1)
})
...
describe('back up buffer is cleared when', () => { // prevent mem leak
  ...
  test('harvest was blocked', () => {
    ...
    expect(softNavAggregate.events.held.buffer.length).toEqual(0)
  })
  test('harvest is sent but got a retry response', () => {
    softNavAggregate.onHarvestFinished({ sent: true, retry: true })
    expect(softNavAggregate.events.held.buffer.length).toEqual(0)
    expect(softNavAggregate.events.buffer.length).toEqual(1) // ixn goes from backup back into the main pending buffer
  })
