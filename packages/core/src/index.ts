...
import { runtime, getConfiguration, setConfiguration, setInfo } from '../../../modules/common/config/config'
...
async function initialize(opts) {
  ...
  if (!initialized && !opts.hasOwnProperty('disable') || (opts.hasOwnProperty('disable') && !opts.disable.includes('errors'))) {
    const { storeError, initialize } = await import('../../../modules/features/js-errors/aggregate')
