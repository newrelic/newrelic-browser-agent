...
let xhrWithAddEventListener = testDriver.Matcher.withFeature(
  "xhrWithAddEventListener"
);
let supported = cors.and(xhrWithAddEventListener);

testDriver.test(
  "events are retried when collector returns 429",
  supported,
...
testDriver.test(
  "multiple custom interactions have correct customEnd value",
  supported,
  function (t, browser, router) {
    ...
    Promise.all([eventsPromise, loadPromise, rumPromise])
      .then(([eventsResult]) => {
        ...
        t.ok(
          qpData.length === 3,
          "three interactions should have been captured"
        );
        qpData.forEach((interaction) => {
          t.ok(
            ["interaction1", "interaction2", "interaction4"].indexOf(
              interaction.customName
            ) > -1,
            "interaction has expected custom name"
          );
          const customEndTime = interaction.children.find(
            (child) => child.type === "customEnd"
          );
          t.ok(
