...
testDriver.test(
  "XHR request on same origin has AJAX request with DT payload",
  supported,
  function (t, browser, router) {
    ...
    let loadPromise = browser.safeGet(
      router.assetURL("spa/dt/xhr-dt-harvest-enabled.html", {
        loader: "spa",
...
testDriver.test(
  "XHR request on different origin has no AJAX request with DT payload",
  supported.and(withCors),
  function (t, browser, router) {
    ...
    let eventsPromise = router.expectEvents();
    let loadPromise = browser.safeGet(
      router.assetURL("spa/dt/xhr-dt-harvest-enabled-different-origin.html", {
        loader: "spa",
        injectUpdatedLoaderConfig: true,
      })
    );
...
testDriver.test(
  ...
  function (t, browser, router) {
    ...
    let eventsPromise = router.expectEvents();
    let loadPromise = browser.safeGet(
      router.assetURL("spa/dt/xhr-dt-harvest-disabled.html", {
        loader: "spa",
        injectUpdatedLoaderConfig: true,
      })
    );
