...
describe('constructor', () => {
  ...
  test('should handle isNew', () => {
    ...
    const newStorage = new LocalMemory({ [`${PREFIX}_${key}`]: { value, expiresAt: Infinity, inactiveAt: Infinity, updatedAt: Date.now(), sessionReplay: 0, sessionTraceActive: false, custom: {} } })
...
describe('read()', () => {
  ...
  test('"pre-existing" sessions get data from read()', () => {
    const storage = new LocalMemory({ [`${PREFIX}_${key}`]: { value, expiresAt: Infinity, inactiveAt: Infinity, updatedAt: Date.now(), sessionReplay: 0, sessionTraceActive: false, custom: {} } })
    const session = new SessionEntity({ agentIdentifier, key, storage })
