...
function getBrowsers (sauceBrowsers, sample = 4) {
  return supportedBrowsers.reduce((aggregator, name) => {
    const filteredBrowsersList = sauceBrowsers
      ...
      .filter(platformSelector(browserName(name)))
 ...
 */
function platformSelector (desiredBrowser) {
  return (sauceBrowser) => {
    ...
    if (sauceBrowser.short_version < minSupportedVersion(desiredBrowser)) return false
    if (sauceBrowser.short_version > maxSupportedVersion(desiredBrowser)) return false
