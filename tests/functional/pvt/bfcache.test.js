...
testDriver.test(
  "Agent doesn't block page from back/fwd cache",
  bfCacheSupport,
  function (t, browser, router) {
    ...
    const assetURL = router.assetURL("instrumented.html", {
      loader: "spa",
      init,
      scriptString,
    });
    ...
    Promise.all([loadPromise, router.expectRum()])
      ...
      .then((pActPayload) => {
        ...
        t.equal(
          pActsReceived[0].actionName,
          "pageCached",
          "page successfully stored in bf cache"
        );
        t.end();
      })
      .catch(fail(t));
...
testDriver.test(
  "EOL events are sent appropriately",
  excludeIE,
  function (t, browser, router) {
    const init = {
    ...
    Promise.all([loadPromise, router.expectRum()])
      ...
      .then((pvtPayload) => {
        ...
        const phTimings = querypack.decode(
          pvtPayload?.body?.length ? pvtPayload.body : pvtPayload.query.e
        );
      ...
      .then((pvtPayload) => {
        ...
        const ulTimings = querypack.decode(
          pvtPayload?.body?.length ? pvtPayload.body : pvtPayload.query.e
        );
        ...
        t.equal(
          ulTimings[0].name,
          "unload",
          "window pagehide emits the unload event (but not our pageHide again)"
        );
        t.end();
      })
      .catch(fail(t));
