    ...
    Promise.all([eventsPromise, rumPromise, loadPromise])
      ...
      .then(({ query, body }) => {
        ...
        t.equal(
          xhr.requestedWith,
          "XMLHttpRequest",
          "should indicate it was requested with xhr"
        );
...
testDriver.test(
  "capturing SPA interactions using loader_config data",
  supported,
  function (t, browser, router) {
    t.plan(23);
    let testStartTime = now();
...
testDriver.test(
  "child nodes in SPA interaction does not exceed set limit",
  supported,
  function (t, browser, router) {
    let rumPromise = router.expectRum();
    let eventsPromise = router.expectEvents();
    let loadPromise = browser.safeGet(
      router.assetURL("spa/fetch-exceed-max-spa-nodes.html", { loader: "spa" })
    );
    ...
    Promise.all([eventsPromise, rumPromise, loadPromise])
      ...
      .then(({ query, body }) => {
        ...
        t.ok(
          interactionTree.children.length <= 128,
          "interaction should have no more than 128 child nodes"
        );
        t.end();
      })
      .catch(fail);
