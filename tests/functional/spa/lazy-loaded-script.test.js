...
testDriver.test(
  "interactions wait for external scripts to complete",
  supported,
  function (t, browser, router) {
    ...
    loadAndWaitForLoad(
      router,
      browser,
      "spa/external-scripts/lazy-loaded-script.html"
    )
      .then(() => {
        ...
        let domPromise = browser
          ...
          .click()
          .waitFor(
            asserters.jsCondition("window.globalCallbackDone", true),
            testDriver.timeout
          );
      ...
      .then(({ query, body }) => {
        ...
        t.ok(
          ...
          "external script should not increase duration of the interaction"
        );
...
testDriver.test(
  "ajax call in loaded script extends interaction",
  supported,
  function (t, browser, router) {
    loadClickAndGetInteraction(
      router,
      browser,
      "spa/external-scripts/script-with-ajax.html"
    )
      .then(({ query, body }) => {
        let i = querypack.decode(body && body.length ? body : query.e)[0];
        ...
        t.ok(i.end, "end is present");
        t.ok(
          i.end >= i.children[0].end,
          "interaction end should be same or higher than the delayed ajax"
        );
...
testDriver.test("errored script", supported, function (t, browser, router) {
  loadClickAndGetInteraction(
    router,
    browser,
    "spa/external-scripts/aborted-script.html"
  )
    .then(({ query, body }) => {
      let i = querypack.decode(body && body.length ? body : query.e)[0];
      ...
      t.ok(
        duration > 0 && duration < 1000,
        "ajax in script should increase duration of the interaction"
      );
