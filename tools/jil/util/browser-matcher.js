...
BrowserMatcher.prototype.intersect = function (other) {
  if (this.finalized || other.finalized)
    throw new Error("Cannot intersect finalized matchers");
...
BrowserMatcher.withFeature = function (feature) {
  ...
  if (!featureMatcher)
    throw new Error(`unrecognized browser feature '${feature}'`);
...
features.workingSendBeacon = features.sendBeacon.and(
  features.brokenSendBeacon.inverse()
);
...
features.notSafariWithSeleniumBug = new BrowserMatcher().exclude(
  "safari",
  ">=13"
);
...
features.supportsFirstInteraction = features.addEventListener.exclude(
  "ie",
  "9"
);
...
features.workerStackSizeGeneratesError = new BrowserMatcher().exclude(
  "firefox"
);
...
features.sharedWorkersFull = features.sharedWorkers.and(
  features.supportESMWorkers
);
