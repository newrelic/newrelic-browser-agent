...
export class Aggregate extends AggregateBase {
  ...
  getScheme() {
    return getConfigurationValue(this.agentIdentifier, "ssl") === false
      ? "http"
      : "https";
  }
  ...
  sendRum() {
    ...
    if (info.queueTime)
      this.aggregator.store("measures", "qt", { value: info.queueTime });
    if (info.applicationTime)
      this.aggregator.store("measures", "ap", { value: info.applicationTime });
    ...
    var measuresQueryString = mapOwn(
      measuresMetrics,
      function (metricName, measure) {
    ...
    chunksForQueryString.push(
      param("af", Object.keys(agentRuntime.features).join(","))
    );
      ...
      if (entries && entries.length > 0) {
        entries.forEach(function (entry) {
          ...
          if (entry.name === "first-paint") {
            chunksForQueryString.push(
              param("fp", String(Math.floor(entry.startTime)))
            );
          } else if (entry.name === "first-contentful-paint") {
            chunksForQueryString.push(
              param("fcp", String(Math.floor(entry.startTime)))
            );
          }
    ...
    chunksForQueryString.push(
      param("ja", customJsAttributes === "{}" ? null : customJsAttributes)
    );
