...
function loaderTypeSupportabilityMetric(loaderType) {
  testDriver.test(
    `generic agent info is captured - ${loaderType}`,
    fetchBrowsers,
    function (t, browser, router) {
      ...
      Promise.all([metricsPromise, rumPromise, loadPromise])
        .then(([data]) => {
          ...
          const loaderTypeSM = supportabilityMetrics.find((x) =>
            x.params.name.includes("LoaderType")
          );
          t.ok(
          ...
          t.ok(
            loaderTypeSM.params.name.includes(loaderTypesMapped[loaderType]),
            `LoaderType SM matches ${loaderType}`
          );
          t.end();
        })
...
testDriver.test(
  "Calling a newrelic[api] fn creates a supportability metric",
  withUnload,
  function (t, browser, router) {
    t.plan(asyncApiFns.length + 6 + NUM_POLYFILL_SM_FEATS);
    let rumPromise = router.expectRum();
    let loadPromise = browser.get(
      router.assetURL("api/customMetrics.html", {})
    );
    let metricsPromise = router.expectMetrics();
    ...
    Promise.all([metricsPromise, rumPromise, loadPromise])
      .then(([data]) => {
        ...
        t.ok(
          observedAPImetrics.length === asyncApiFns.length,
          "Saw all asyncApiFns"
        );
...
testDriver.test(
  "a valid obfuscationRule creates detected supportability metric",
  fetchBrowsers,
  function (t, browser, router) {
    let rumPromise = router.expectRum();
    let metricsPromise = router.expectMetrics();
    const loadPromise = browser.safeGet(
      router.assetURL("obfuscate-pii-valid.html", {
        loader: "spa",
      })
    );
...
testDriver.test(
  "an invalid obfuscation regex type creates invalid supportability metric",
  fetchBrowsers,
  function (t, browser, router) {
    let rumPromise = router.expectRum();
    let metricsPromise = router.expectMetrics();
    const loadPromise = browser.safeGet(
      router.assetURL("obfuscate-pii-invalid-regex-type.html", {
        loader: "spa",
      })
    );
...
testDriver.test(
  "an invalid obfuscation regex undefined creates invalid supportability metric",
  fetchBrowsers,
  function (t, browser, router) {
    let rumPromise = router.expectRum();
    let metricsPromise = router.expectMetrics();
    const loadPromise = browser.safeGet(
      router.assetURL("obfuscate-pii-invalid-regex-undefined.html", {
        loader: "spa",
      })
    );
...
testDriver.test(
  "an invalid obfuscation replacement type creates invalid supportability metric",
  fetchBrowsers,
  function (t, browser, router) {
    let rumPromise = router.expectRum();
    let metricsPromise = router.expectMetrics();
    const loadPromise = browser.safeGet(
      router.assetURL("obfuscate-pii-invalid-replacement-type.html", {
        loader: "spa",
      })
    );
...
testDriver.test("workers creation generates sm", function (t, browser, router) {
  ...
  let metricsPromise = router.expectMetrics();
  const loadPromise = browser.safeGet(
    router.assetURL("instrumented-worker.html", {
      loader: "spa",
    })
  );
  ...
  Promise.all([metricsPromise, rumPromise, loadPromise])
    .then(([data]) => {
      ...
      let workerShouldExistOnThisBrowser = testDriver.Matcher.withFeature(
        "workers"
      ).match(browser.browserSpec);
      if (workerShouldExistOnThisBrowser && !wsm.workerImplFail) {
        // worker may be avail in Chrome v4, but our SM implementation may not be supported until v60, etc.
      ...
      workerShouldExistOnThisBrowser = testDriver.Matcher.withFeature(
        "sharedWorkers"
      ).match(browser.browserSpec);
      if (workerShouldExistOnThisBrowser && !wsm.sharedImplFail) {
        ...
        t.ok(
          ...
          "sharedworker API or SM should be unavailable on this browser version"
        );
      }
      ...
      if (workerShouldExistOnThisBrowser) {
        t.notOk(
          ...
          "classic or module serviceworker is NOT expected or used"
        );
        t.ok(
          wsm.serviceUnavail || wsm.serviceImplFail,
          "serviceworker API should be unavailable on all"
        );
      }
