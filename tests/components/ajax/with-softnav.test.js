...
test('on interaction finish, ajax within time span are associated, others standalone', () => {
  ...
  ajaxAggregate.storeXhr(params, metrics, 175, 225, 'XMLHttpRequest', undefined, context) // ensure xhr happening during default heuristics are associated
  softNavAggregate.ee.emit('newDom', [200])
  expect(softNavAggregate.interactionInProgress).toBeTruthy()

  ajaxAggregate.storeXhr(params, metrics, 250, 350, 'fetch', undefined, context) // ensure xhr happening during wait window for actualized long tasks are associated
  ajaxAggregate.storeXhr(params, metrics, 325, 400, 'XMLHttpRequest', undefined, context) // this one should fall outside the final interaction span and be standalone
...
test('on spa API .end(), ajax prior to call is associated and post-call is standalone', () => {
  ...
  ajaxAggregate.storeXhr(params, metrics, 250, 299, 'fetch', undefined, context)
  softNavAggregate.interactionInProgress.done(300, true) // simulate API call to .end() at 300, this would occur in order
  ajaxAggregate.storeXhr(params, metrics, 301, 350, 'fetch', undefined, context)
