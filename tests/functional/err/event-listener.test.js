...
let supported = reliableUnloadEvent.exclude("ie@<9").exclude("firefox@<4");
    ...
    Promise.all([rumPromise, loadPromise])
      .then(([response]) => {
        ...
        actualErrors.forEach((err, i) => {
          t.equal(
            err.params.exceptionClass,
            "Error",
            err.params.message + "exceptionClass is Error"
          );
          t.equal(
            err.params.message,
            expectedErrors[i].message,
            err.params.message + "message is correct"
          );
          t.ok(
            err.params.stack_trace.includes("handleEvent"),
            "Stack trace has handleEvent"
          );
          t.ok(
            err.params.stack_trace.includes(eventListenersURL),
            "Stack trace has eventListenerUrl"
          );
        });
