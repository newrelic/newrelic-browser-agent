...
function metricsApiCreatesSM (type, browserVersionMatcher) {
  testDriver.test(
    `${type} - Calling a newrelic[api] fn creates a supportability metric`,
      ...
      Promise.all([metricsPromise, loadPromise])
        .then(([data]) => {
          ...
          for (const sm of supportabilityMetrics) {
            ...
            { t.equal(sm.stats.c, 5, sm.params.name + ' count was incremented by 1 until reached 5') } else if (sm.params.name.startsWith('Workers/'))
            // these metrics have a dynamic count & are tested separately anyways
            { continue } else t.equal(sm.stats.c, 1, sm.params.name + ' count was incremented by 1') // there should be 1 generic sm for agent version--extra #3
          }
...
function metricsValidObfuscationCreatesSM (type, browserVersionMatcher) {
  testDriver.test(
    `${type} - a valid obfuscationRule creates detected supportability metric`,
    browserVersionMatcher,
    function (t, browser, router) {
      let assetURL = router.assetURL(`worker/${type}-worker.html`, {
        init: {
