...
export class Instrument extends InstrumentBase {
  ...
  constructor(agentIdentifier, aggregator, auto = true) {
    ...
    this.handler = (type, args, ctx, group) =>
      handle(type, args, ctx, group, this.ee);
...
function subscribeToEvents(agentIdentifier, ee, handler, dt) {
  ...
  function onAddEventListenerEnd(args, xhr) {
    if (xhr instanceof origXHR && args[0] === "load")
      ee.emit("xhr-load-added", [args[1], args[2]], xhr);
  }

  function onRemoveEventListenerEnd(args, xhr) {
    if (xhr instanceof origXHR && args[0] === "load")
      ee.emit("xhr-load-removed", [args[1], args[2]], xhr);
  }
  ...
  function onFnStart(args, xhr, methodName) {
    if (xhr instanceof origXHR) {
      ...
      if ((args[0] && args[0].type) === "load" || this.onload)
        this.xhrCbStart = now();
    }
  }
  ...
  function onFnEnd(args, xhr) {
    if (this.xhrCbStart)
      ee.emit("xhr-cb-time", [now() - this.xhrCbStart, this.onload, xhr], xhr);
  }
  ...
  function onFetchBeforeStart(args) {
    ...
    if (
      !payload ||
      (!payload.newrelicHeader && !payload.traceContextParentHeader)
    ) {
      return;
    }
    ...
    if (
      ...
      (globalScope?.URL && args[0] && args[0] instanceof URL)
    ) {
      var clone = {};
  ...
  function onFetchStart(fetchArguments, dtPayload) {
    ...
    var method = (
      "" +
      ((target && target instanceof origRequest && target.method) ||
        opts.method ||
        "GET")
    ).toUpperCase();
  ...
  function onFetchDone(err, res) {
    ...
    handler(
      "xhr",
      [this.params, metrics, this.startTime, this.endTime, "fetch"],
      this,
      FEATURE_NAMES.ajax
    );
  }
  ...
  function end(xhr) {
    ...
    handler(
      "xhr",
      [params, metrics, this.startTime, this.endTime, "xhr"],
      this,
      FEATURE_NAMES.ajax
    );
  }
