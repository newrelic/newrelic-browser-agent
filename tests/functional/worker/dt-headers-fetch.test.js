...
function fetchDTHeader (type, testCase, browserVersionMatcher) {
  testDriver.test(`${type} - ${testCase.name}`, browserVersionMatcher, function (t, browser, router) {
    const config = {
      accountID: '1234',
      agentID: '2468',
      trustKey: '1'
    ...
    scenarios.forEach((scenario) => {
      t.test(`${type} - ${scenario.name}`, (t) => {
        let assetURL = router.assetURL(`worker/${type}-worker.html`, {
          ...
          workerCommands: [`self.testId = '${router.testID}'`, scenario.fetchTest].map((x) => x.toString())
        })

        const loadPromise = browser.get(assetURL)
        const fetchPromise = router.expectCustomGet('/dt/{key}', (req, res) => {
          res.end('ok')
        })
