...
testDriver.test(
  ...
  function (t, browser, router) {
    waitForPageLoadAnInitialCalls(
      browser,
      router,
      "spa/errors/captured-initial-page-load.html"
    )
      ...
      .then(([eventData, errorData]) => {
        ...
        t.ok(
          ...
          "interaction should have nodeId attribute"
        );
        ...
        t.equal(
          error.params.browserInteractionId,
          interactionId,
          "should have the correct interaction id"
        );
  ...
  waitForPageLoadAnInitialCalls(browser, router, "spa/errors/captured-xhr.html")
    ...
    .then(([eventData, errorData]) => {
      ...
      t.equal(
        error.params.parentNodeId,
        nodeId,
        "has the correct parent node id"
      );
      t.equal(
        error.metrics.count,
        1,
        "error should have been reported only once"
      );
...
testDriver.test("error in custom tracer", function (t, browser, router) {
  waitForPageLoadAnInitialCalls(
    browser,
    router,
    "spa/errors/captured-custom.html"
  )
    .then(() => {
      return clickPageAndWaitForEventsAndErrors(t, browser, router);
    })
...
testDriver.test("string error in custom tracer", function (t, browser, router) {
  ...
  waitForPageLoadAnInitialCalls(
    browser,
    router,
    "spa/errors/captured-custom-string.html"
  )
    .then(() => {
      return clickPageAndWaitForEventsAndErrors(t, browser, router);
    })
    .then(([eventData, errorData]) => {
      ...
      if (browser.match("ie@>=11"))
        t.equal(
          errors.length,
          2,
          "should have 2 errors (1 String Class, 1 Error Class)"
        );
...
testDriver.test(
  "errors in discarded SPA interactions",
  function (t, browser, router) {
    waitForPageLoadAnInitialCalls(
      browser,
      router,
      "spa/errors/discarded-interaction.html"
    )
      .then(() => {
      ...
      .then(([domData, errorData]) => {
        ...
        t.equal(
          error.params.browserInteractionId,
          undefined,
          "should not have interaction id"
        );
        t.equal(
          error.params.parentNodeId,
          undefined,
          "should not have parent node id"
        );
...
testDriver.test("errors outside of interaction", function (t, browser, router) {
  waitForPageLoadAnInitialCalls(
    browser,
    router,
    "spa/errors/captured-nointeraction.html"
  )
    .then(() => {
      var p = clickAndRedirect(browser, router, 200); // wait 200ms because the test page itself waits 100ms before throwing error
      return Promise.all([p, router.expectErrors()]);
...
testDriver.test(
  "same error in multiple interactions",
  function (t, browser, router) {
    var event1;
    var event2;
    ...
    waitForPageLoadAnInitialCalls(
      ...
      .then((errorData) => {
        let interaction1 = querypack.decode(
          event1.body && event1.body.length ? event1.body : event1.query.e
        )[0];
        ...
        let interaction2 = querypack.decode(
          event2.body && event2.body.length ? event2.body : event2.query.e
        )[0];
