...
describe('pvt aggregate tests', () => {
  ...
  test('LCP event with CLS attribute', () => {
    const timing = find(pvtAgg.events.buffer, function (t) {
      return t.name === 'lcp'
    })
  ...
  test('sends expected FI attributes when available', () => {
    expect(pvtAgg.events.hasData).toEqual(true)
    const fiPayload = pvtAgg.events.buffer.find(x => x.name === 'fi')
  ...
  test('sends CLS node with right val on vis change', () => {
    let clsNode = pvtAgg.events.buffer.find(tn => tn.name === VITAL_NAMES.CUMULATIVE_LAYOUT_SHIFT)
    expect(clsNode).toBeUndefined()

    triggerVisChange()
    clsNode = pvtAgg.events.buffer.find(tn => tn.name === VITAL_NAMES.CUMULATIVE_LAYOUT_SHIFT)
  ...
  test('sends INP node with right val', () => {
    let inpNode = pvtAgg.events.buffer.find(tn => tn.name === VITAL_NAMES.INTERACTION_TO_NEXT_PAINT)
