...
jil.browserTest(
  "fn-start events for MutationObserver callbacks should have args",
  function (t) {
    t.plan(3);
    ...
    var observer = new MutationObserver(function (mutationRecords, o) {
      ...
      t.equal(
        o,
        observer,
        "observer received in callback matches original observer"
      );
    });
    ...
    baseEE.on("fn-start", function (args) {
      t.equal(
        args.length,
        2,
        "fn-start event gets MutationObserver callback args"
      );
    });
