...
function xhrErrorTest(type, matcher) {
  testDriver.test(
    `${type} - an error in xhr callback is noticed and harvested`,
    matcher,
      ...
      Promise.all([xhrPromise, loadPromise])
        .then(([response]) => {
          t.equal(
            response.req.method,
            "POST",
            "XHR data submitted via POST request from sendBeacon"
          );
          ...
          t.deepEqual(
            ["metrics", "params"],
            Object.keys(parsedXhrs[0]).sort(),
            "XHR record has correct keys"
          );
          t.end();
        })
        .catch(fail(t));
    ...
    function (t, browser, router) {
      ...
      Promise.all([xhrPromise, loadPromise])
        .then(([response]) => {
          ...
          const parsedXhrs = getXhrFromResponse(response, browser);
          var fetchData = parsedXhrs.find(
            (xhr) => xhr.params.pathname === "/json"
          );
          t.ok(fetchData, "has xhr data");
          t.deepEqual(
            ["metrics", "params"],
            Object.keys(fetchData).sort(),
            "XHR record has correct keys"
          );
          t.end();
        })
