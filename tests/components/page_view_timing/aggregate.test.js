...
test('LCP event with CLS attribute', () => {
  const timing = find(timingsAggregate.timings.events, function (t) {
    return t.name === 'lcp'
  })
...
test('sends expected FI attributes when available', () => {
  expect(timingsAggregate.timings.events.length).toBeGreaterThanOrEqual(1)
  const fiPayload = timingsAggregate.timings.events.find(x => x.name === 'fi')
...
test('sends CLS node with right val on vis change', () => {
  let clsNode = timingsAggregate.timings.events.find(tn => tn.name === VITAL_NAMES.CUMULATIVE_LAYOUT_SHIFT)
  expect(clsNode).toBeUndefined()

  pageVisibilityModule.subscribeToVisibilityChange.mock.calls[1][0]()

  clsNode = timingsAggregate.timings.events.find(tn => tn.name === VITAL_NAMES.CUMULATIVE_LAYOUT_SHIFT)
...
test('sends INP node with right val', () => {
  let inpNode = timingsAggregate.timings.events.find(tn => tn.name === VITAL_NAMES.INTERACTION_TO_NEXT_PAINT)
