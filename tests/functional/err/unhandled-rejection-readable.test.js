...
testDriver.test(
  "unhandledPromiseRejections are caught and are readable",
  supported,
  function (t, browser, router) {
    let assetURL = router.assetURL(
      "unhandled-promise-rejection-readable.html",
      {
        init: {
          page_view_timing: {
            enabled: false,
    ...
    Promise.all([errorPromise, rumPromise, loadPromise])
      .then(([response]) => {
        ...
        actualErrors.forEach((err) => {
          const targetError = expectedErrorMessages.find(
            (x) => !x.tested && x.message === err.params.message
          );
          if (targetError) targetError.tested = true;
          t.ok(
            !!targetError,
            `expected ${targetError?.meta} message exists (${err.params.message})`
          );
