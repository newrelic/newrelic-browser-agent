...
describe('constructor', () => {
  ...
  test('expiresAt is the correct future timestamp - existing session', () => {
    const now = Date.now()
    jest.setSystemTime(now)
    const existingData = new LocalMemory({ [`${PREFIX}_${key}`]: { value, expiresAt: now + 5000, inactiveAt: Infinity, updatedAt: now, sessionReplayMode: 0, sessionReplaySentFirstChunk: false, sessionTraceMode: 0, custom: {} } })
  ...
  test('inactiveAt is the correct future timestamp - existing session', () => {
    const now = Date.now()
    jest.setSystemTime(now)
    const existingData = new LocalMemory({ [`${PREFIX}_${key}`]: { value, inactiveAt: now + 5000, expiresAt: Infinity, updatedAt: now, sessionReplayMode: 0, sessionReplaySentFirstChunk: false, sessionTraceMode: 0, custom: {} } })
  ...
  test('should handle isNew', () => {
    ...
    const newStorage = new LocalMemory({ [`${PREFIX}_${key}`]: { value, expiresAt: Infinity, inactiveAt: Infinity, updatedAt: Date.now(), sessionReplayMode: 0, sessionReplaySentFirstChunk: false, sessionTraceMode: 0, custom: {} } })
...
describe('read()', () => {
  ...
  test('"pre-existing" sessions get data from read()', () => {
    const storage = new LocalMemory({ [`${PREFIX}_${key}`]: { value, expiresAt: Infinity, inactiveAt: Infinity, updatedAt: Date.now(), sessionReplayMode: 0, sessionReplaySentFirstChunk: false, sessionTraceMode: 0, custom: {} } })
    const session = new SessionEntity({ agentIdentifier, key, storage })
