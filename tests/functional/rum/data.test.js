  ...
  Promise.all([rumPromise, loadPromise])
    .then(([{ query }]) => {
      ...
      if (browser.match("firefox@<=10")) {
        t.skip(
          "old version of firefox dont report Dom content loaded correctly"
        );
      } else {
        t.ok(
          +query.dc > 0,
          "DOM content loaded time of " + query.dc + " is > 0"
        );
      }
...
testDriver.test(
  "rum data with multiple app IDs",
  withTls,
...
testDriver.test(
  "rum data using loader_config data",
  withTls,
  function (t, browser, router) {
    t.plan(13);
    ...
    Promise.all([rumPromise, loadPromise])
      .then(([{ query }]) => {
        ...
        t.equal(
          query.ja,
          '{"foo":"bar"}',
          "custom javascript attributes params"
        );
...
testDriver.test(
  "rum data with multiple app IDs using loader_config data",
  withTls,
  function (t, browser, router) {
    let config = {
      applicationID: "1234,5678",
