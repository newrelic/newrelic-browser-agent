    ...
    Promise.all([metricsPromise, rumPromise, loadPromise])
      .then(([data]) => {
        ...
        for (const sm of supportabilityMetrics) {
          ...
          if (sm.params.name === multipleApiCalls) { t.equal(sm.stats.c, 5, sm.params.name + ' count was incremented by 1 until reached 5') } else if (sm.params.name.startsWith('Workers/')) {
