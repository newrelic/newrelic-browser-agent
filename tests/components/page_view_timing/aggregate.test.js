...
test('LCP event with CLS attribute', () => {
  const timing = find(timingsAggregate.events.buffer, function (t) {
    return t.name === 'lcp'
  })
...
test('sends expected FI attributes when available', () => {
  expect(timingsAggregate.events.buffer.length).toBeGreaterThanOrEqual(1)
  const fiPayload = timingsAggregate.events.buffer.find(x => x.name === 'fi')
...
test('sends CLS node with right val on vis change', () => {
  let clsNode = timingsAggregate.events.buffer.find(tn => tn.name === VITAL_NAMES.CUMULATIVE_LAYOUT_SHIFT)
  expect(clsNode).toBeUndefined()

  pageVisibilityModule.subscribeToVisibilityChange.mock.calls[1][0]()

  clsNode = timingsAggregate.events.buffer.find(tn => tn.name === VITAL_NAMES.CUMULATIVE_LAYOUT_SHIFT)
...
test('sends INP node with right val', () => {
  let inpNode = timingsAggregate.events.buffer.find(tn => tn.name === VITAL_NAMES.INTERACTION_TO_NEXT_PAINT)
