...
const bucketMap = {
  ...
  ins: [FEATURE_NAMES.pageAction, FEATURE_NAMES.marksAndMeasures],
...
test('emits the right events when feature flag = 1', () => {
  ...
  // each flag gets emitted to each of its mapped features, and a feat- AND a rumresp- for every emit, so (1+2+2+1+2)*2 = 16
...
test('emits the right events when feature flag = 0', () => {
  ...
  // each flag gets emitted to each of its mapped features, and a block- AND a rumresp- for every emit, so (1+2+2+1+2)*2 = 16
  expect(handleModule.handle).toHaveBeenCalledTimes(16)
