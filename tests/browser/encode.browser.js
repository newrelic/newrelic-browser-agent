...
test("encode.fromArray", function (t) {
  ...
  t.equal(
    encode.fromArray(doubleTest, 5),
    "aabb",
    "Fall back to largest whole chunk that wokrs"
  );
  t.end();
});
...
test("encode.obj", function (t) {
  ...
  t.equal(
    encode.obj(obj3, 1000),
    "&bar=%5B%22a%22,null,%22c%22%5D",
    "Handle bad objects"
  );
...
test("encode.param", function (t) {
  t.equal(
    encode.param("foo", null),
    "",
    "Empty string for things without a value"
  );
  t.equal(
    encode.param("foo", "bar"),
    "&foo=bar",
    "formed key value for two strings"
  );
